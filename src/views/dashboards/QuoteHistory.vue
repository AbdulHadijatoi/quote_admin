...
import { CalendarIcon, DownloadIcon, DetailsIcon, LiveViewIcon, View360Icon, ViewfinderIcon, EyeIcon, PlusIcon, TargetIcon, LinkIcon } from 'vue-tabler-icons'; // Importing icons from vue-tabler-icons
// Import your postData function
import { postData, getPdf, postPdf } from '@/utils/api';
...
const preview = async (id: number) => {
  isLoading.value = true;
  await getPdf('/shipping-quotes/preview-pdf/'+id );
  isLoading.value = false;
};
...
const download = async (id: number) => {
  const formData = new FormData();
  isLoading.value = true;
  await postPdf('/shipping-quotes/preview-pdf/'+id, formData);
  isLoading.value = false;
};
        ...
        <v-card-text>
          ...
          <v-data-table-server
            :headers="[
              ...
              { title: 'Generated by Employee', key: 'generated_by_employee' },
          ...
          >
            <template v-slot:item.actions="{ item }">
              <v-btn icon @click="viewDetails(item.id)" elevation="0"><EyeIcon size="20"/>
                <v-tooltip activator="parent" location="top">View</v-tooltip>
              </v-btn>
              <v-btn icon @click="download(item.id)" elevation="0"><DownloadIcon size="20"/>
                <v-tooltip activator="parent" location="top">Download</v-tooltip>
              </v-btn>
              <v-btn icon @click="preview(item.id)" elevation="0"><LinkIcon size="20"/>
                <v-tooltip activator="parent" location="top">Preview Quote</v-tooltip>
              </v-btn>
            </template>
          </v-data-table-server>
        </v-card-text>
